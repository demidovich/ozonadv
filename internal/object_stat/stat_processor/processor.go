	...
	go func() {
		...
		for campaign := range in {
			...
			if campaign.Stat.UUID != "" {
				...
				p.storage.ObjectStatCampaigns().Add(campaign)
			}
...
func (p *statProcessor) readyStatRequestsStage(in <-chan ozon.StatRequest) <-chan ozon.StatRequest {
	...
	go func() {
		...
		for r := range in {
			...
			if campaign, ok := p.storage.ObjectStatCampaigns().ByStatUUID(statRequest.UUID); ok {
...
func (p *statProcessor) createStatRequest(campaign ozon.Campaign) ozon.StatRequest {
	options := ozon.CreateStatRequestOptions{
		...
		DateFrom:   p.storage.ObjectStatOptions().DateFrom,
		DateTo:     p.storage.ObjectStatOptions().DateTo,
		GroupBy:    p.storage.ObjectStatOptions().GroupBy,
	}
